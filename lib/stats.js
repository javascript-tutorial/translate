...
module.exports = class Stats {
  ...
  async gather(langCode) {
    ...
    let lastCommit = await request({
      url:     `https://api.github.com/repos/${config.org}/${langCode}.${config.repoSuffix}/commits/master`,
      headers: {
        'User-Agent':    'javascript.info',
        'Accept':        'application/vnd.github.v3+json',
        'Authorization': `token ${config.secret.github.token}`
    ...
    this.set(langCode, {translation, contributors, repo, lastCommit});
