...
/*
  Checks out / updates repos w/ translations
...
*/
  ...
  module.exports = async function(repoOriginal, repoTranslated) {

  debug(`Compare ${repoOriginal} vs ${repoTranslated}`);

  let filesFrom = glob.sync('**/*.md', {cwd: repoOriginal}).filter(f => parseInt(f));
  ...
  for (let relPath of filesFrom) {
    ...
    if (repoOriginal === repoTranslated) {
      filesTranslated++;
      translated[getUrlBy(relPath)] = 1;
      continue;
    }

    let fileFrom = path.join(repoOriginal, relPath);
    let fileLang = path.join(repoTranslated, relPath);
  ...
  debug("PROGRESS unrounded", progress);
