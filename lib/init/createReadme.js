...
module.exports = async function(langInfo) {
  ...
  await run(`git fetch origin master`, {cwd: translatedPath});
  await run(`git checkout -f -B master origin/master`, {cwd: translatedPath});
  await run(`git clean -df`, {cwd: translatedPath});
